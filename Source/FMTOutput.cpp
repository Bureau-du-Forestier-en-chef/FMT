	...
	for (std::size_t i = 0; i < p_source.size(); ++i) {
		try
		{
			...
			else
			{
				...
				/*if (count > MAXSIZE)
				{
					const std::string ERRORMESSAGE = "Le nombre de carat�res par ligne est trop grand. Nombre de caract�res : " + std::to_string(count) + " nombre max : " + std::to_string(MAXSIZE);
...
std::string FMTOutput::_operatorFormat() const
{
	...
	newLine += " \n";
	newLine += "*SOURCE ";
	// On regarde le dernier double op * ou / pour ajuster les outputs en cons�quence
