...
namespace Parser {
	...
	std::string FMTscheduleparser::getSchedulePath(const std::string& p_primary_path, const std::string& p_output_scenario_name) 
	{
		std::string thePath;
		try {
			// On v�rifie si le dossier Scenarios existe et si le fichier de schedule existe, sinon on �crit dans le root
			...
			// On construit notre path attendu et v�rifie s'il existe dans le dossier Scenarios / notre_scenario
